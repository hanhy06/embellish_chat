...
| Link              | `[Text](URL)`       | `[Google](https://google.com)`  | <span style="color:#0000EE;"><u>Google</u></span>           |
...
You can find `embellish_chat.json` in your save folder.
...
* **Mod Does Not Load:**
  ...
  2. Confirm that Fabric API and EmbellishChat `.jar` files are both in the `mods` folder.
  3. Check the server/client logs for errors related to EmbellishChat during startup.

* **Color Formatting Not Applying:**

  1. Verify `textPostProcessingEnabled` is set to `true` in `embellish_chat.json`.
